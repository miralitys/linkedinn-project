...
async def list_companies(session: AsyncSession = Depends(get_session)):
    r = await session.execute(select(Company).order_by(func.lower(Company.name)))
...
async def get_company(id: int, session: AsyncSession = Depends(get_session)):
...
async def update_company(id: int, body: CompanyUpdate, session: AsyncSession = Depends(get_session)):
...
async def delete_company(id: int, session: AsyncSession = Depends(get_session)):
    c = await session.get(Company, id)
    if not c:
        raise HTTPException(404, "Company not found")
