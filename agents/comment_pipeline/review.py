...
def rule_compliance_check(
    ...
    # Punctuation: no em dash, no colon — only periods and commas
...
async def review_draft(
    ...
    rule_flags = rule_compliance_check(draft, policy, product_plan, products, mode)
    ...
    if not prompt_tpl:
        return _quick_review(draft, variant, policy, product_plan, products, mode)
    ...
    try:
        ...
        if isinstance(data, dict):
            ...
            fail_flags = {"no_post_anchor", "fake_personal_claim", "lecture_mode", "toxicity", "em_dash", "colon"}
            if mode == MODE_NETWORK:
...
def _quick_review(
    ...
    flags.extend(rule_compliance_check(draft, policy, product_plan, products or [], mode))
    return {
        "pass": len(flags) == 0,
